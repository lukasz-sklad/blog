...
<h2>Generowanie klucza SSH</h2>

`ssh-keygen -t rsa -b 4096 -C "lukasz.sklad@gmail.com"`
...
Klucza SSH dodajemy, wybierając 'New SSH key’. Pokaże nam się pole 'Title’ oraz 'Key’. W pierwsze wpisujemy nazwę, a w drugie wklejamy nasz klucz ze schowka. Na końcu za pomocą przycisku 'Add SSH key’ zapisujemy klucz.
...
Hi lukasz-sklad! You've successfully authenticated, but GitHub does not provide shell access.

<h3>Ustawienie gita aby działał przez ssh:</h3>

```git remote set-url origin git@github.com:lukasz-sklad/blog.git```
